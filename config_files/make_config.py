...
3/11/19: changed recording_sys to TDT from plexon
'''
import os
...
stuff['recording_sys'] = dict(make='Ripple', mount_point='/media/samantha/ssd/storage/ripple')
...
for dbname in databases:
    stuff['db_config_%s' % dbname] = dict(data_path='/media/samantha/ssd/storage')
...
for system_name, system_opts in stuff.items():
    ...
    for option, default in system_opts.items():
        ...
        opt_val = 0input("Enter value for '%s' (default=%s): " % (option, str(default)))
        if opt_val == '':
            opt_val = default
        config_fh.write('%s = %s\n' % (option, opt_val))
    config_fh.write('\n')
