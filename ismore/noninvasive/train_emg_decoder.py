...
from ismore.noninvasive import emg_decoding, find_relevant_channels
...
# Aqui pone los ids de los files que uiqeres utilizat para entrenar el decoder. Y asegurate que los hdf files estan en la carpeta storgae/raw_data/hdf y los supp files en storage/supp_hdf o algo asi.

train_hdf_ids = [10161,10162,10163,10164,10165]
...
#train_hdf_ids = [4737,4738,4742,4743,4746,4747,6967,6968,6971,6973,6974,6976,6979,6980,6982,6984,6987,6988] # healthy arm
...
# Si quieres testear offline pon los ids aqui.
test_hdf_ids = []
...
# 25/10?2017
#test_hdf_ids = [8368,8369,8372,8373]
# 26/10?2017
...
emg_prono_bip = ['PronTer',
    'Biceps']

emg_upper_arm_bip_filt = [i + '_filt' for i in emg_upper_arm_bip]
emg_thumb_bip_filt = [i + '_filt' for i in emg_thumb_bip]
emg_index_bip_filt = [i + '_filt' for i in emg_index_bip]
emg_fing3_bip_filt = [i + '_filt' for i in emg_fing3_bip]
emg_prono_bip_filt = [i + '_filt' for i in emg_prono_bip]
...
mirrored = False #whether to train a mirrored decoder or not
if mirrored == True:
    states_to_flip = ['aa_vx', 'aa_vpsi'] # if the values of the rehand are positive when we recorded with the left hand (sess03), because we took all of them as possitive
else:
    states_to_flip = []
