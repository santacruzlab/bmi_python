...
class LinearEMGDecoder(EMGDecoderBase):
    ...
    def train_ridge(self, K, train_hdf_names, test_hdf_names, states_to_flip):
        ...
        for hdf_name in all_hdf_names:
            ...
            ts_task = hdf.root.task[:]['ts'].ravel()
            #import pdb; pdb.set_trace() 
            ...
            if 'ts_rh' in locals() and 'ts_aa' in locals() and 'ts_task' in locals():
                ts_max = min(ts_emg[-1], ts_aa[-1], ts_rh[-1], ts_task[-1])
                ts_min = max(ts_emg[0], ts_aa[0], ts_rh[0], ts_task[0])
            elif 'ts_rh' not in locals() and 'ts_aa' in locals():
                if 'ts_task' in locals():
                    ts_max = min(ts_emg[-1], ts_aa[-1], ts_task[-1])
                    ts_min = max(ts_emg[0], ts_aa[0], ts_task[0])
                ts_max = min(ts_emg[len(ts_emg)-1], ts_aa[len(ts_aa)-1])
                ts_min = max(ts_emg[0], ts_aa[0])
            elif 'ts_rh' in locals() and 'ts_aa' not in locals():
                if 'ts_task' in locals():
                    ts_max = min(ts_emg[-1], ts_rh[-1], ts_task[-1])
                    ts_min = max(ts_emg[0], ts_rh[0], ts_task[0])
            ...
            #         if ts_data[0] > ts_features[0] or ts_data[len(ts_data)-1] < ts_features[len(ts_features)-1]:
            #             import pdb; pdb.set_trace()
            ...
            #                 kin_data_test_lpf[vel_state] = kin_data_lpf.copy()
            self.command_vel = True
            if self.command_vel == True:

                for dof, vel_state in enumerate(self.states_to_decode):
                    state_data = hdf.root.task[:]['command_vel'][:,dof].ravel()
                    interp_fn = interp1d(ts_task, state_data)
                    interp_state_data = interp_fn(ts_features)
                    ...
                    if inddiff.size != 0:
                        ...
                        clean_data = interp_state_data[clean_idxs].copy()
                        interp_fn = interp1d(clean_idxs, clean_data)
                        inddiff = inddiff[inddiff<clean_idxs[-1]]
                        print clean_idxs[-1], inddiff[-1]-2
                            #import pdb; pdb.set_trace()
