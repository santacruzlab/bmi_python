...
'''
from scipy.signal import sigtools, lfilter, lfilter_zi
import numpy as np
...
class Filter(object):
    def __init__(self, b=[], a=[1.]):
        ...
        # self.zi0 = np.ones(max(len(a), len(b))-1)

        # self.zi0 = np.zeros(max(len(a), len(b))-1)
        self.zi0 = lfilter_zi(self.b, self.a)
    ...
    def __call__(self, samples):
        ...
        if self.first_iter:
            filt_output, self.zi = lfilter(self.b, self.a, samples, zi=self.zi0*samples[0])
