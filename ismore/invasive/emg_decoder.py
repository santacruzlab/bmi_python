...
class EMGBioFeedback(EMGDecoderBase):
    def __init__(self, baseline, max_activity, rad, mult, **kwargs):
        self.baseline = baseline
        self.max_activity = max_activity
        self.max_base_diff = self.max_activity - self.baseline
        self.rad = rad
        self.mult = mult

    def __call__(self, features):
        # round: 
        tmp = (features - self.baseline) / self.max_base_diff
        tmp = np.max([0, tmp])
        tmp = np.min([1, tmp])

        # map to workspace: 
        decoder_output = (tmp - 0.5) * (2*self.rad)
        decoder_output = self.mult *decoder_output
        return decoder_output

    def train_from_tes(self):
        pass

class EMGBioFeedbackDiff(EMGBioFeedback):
    def __call__(self, features):
        diff = features[0] - features[1]
        tmp = (diff - self.baseline) / self.max_base_diff
        tmp = np.max([0, tmp])
        tmp = np.min([1, tmp])
