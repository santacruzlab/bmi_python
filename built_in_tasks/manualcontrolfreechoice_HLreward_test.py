'''
Base tasks for two targets with different reward probability.
'''
...
"yellow": (1,1,0,0.5),
"hibiscus":(0.859,0.439,0.576,0.75),
"magenta": (1,0,1,0.5),
...
'''
I only changed the first class - debug this before moving on to other tasks. Major changes: (1) deleted check_reward state. this makes yes_reward and no_reward 
transitions obsolete, but I have not deleted them yet (2) reward timer self.reward_time is now set to be either reward_time_high or reward_time_low in the 
_start_hold_targetL and _start_hold_targetH functions (3) reward_time is no longer a settable variable but reward_time_high and reward_time_low are
'''

class ManualControlFreeChoice_HLreward(Sequence, Window):
    '''
    ...
    joystick_method = traits.Float(0,desc="1: Normal velocity, 0: Position control")
    ...
    reward_time_high = traits.Float(.8, desc="High length of juice reward")
    reward_time_low = traits.Float(.4, desc="Low length of juice reward")
    ...
    location_offset_allowed = traits.Float(0, desc="If targets may be at asymmetric distances from center, this is 1. If always equidistant, this is 0.")
    ...
    reward_time = 0  # initial reward time variable that will change between reward_time_high and reward_time_low throughout task
 ...
 ##target_colors = blue, yellow, hibiscus, magenta, purple, 
 ...
 # olive, chiffonlemon, juicyorange, salmon, wood, elephant
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Initialize colors for high probability and low probability target.  Color will not change. #HS Color needs to change
    ...
    def init(self):
        ...
        self.add_dtype('random_rest_time', 'f8', (1,)) #HS
        self.add_dtype('HMS_ts', 'f8', (1,)) #HS
        super(ManualControlFreeChoice_HLreward, self).init()
    ...
    def _cycle(self):
        ...
        super(ManualControlFreeChoice_HLreward, self)._cycle()
    ...
    def move_arm(self):
        ...
        if len(pt) > 0:
            ...
            if self.joystick_method==0:                
                ...
                if self.current_pt[0] < -5: self.current_pt[0] = -14
                if self.current_pt[0] > 5: self.current_pt[0] = 14
                if self.current_pt[-1] < 0: self.current_pt[-1] = 0
                if self.current_pt[-1] > 0: self.current_pt[-1] = 0
    ...
    def update_report_stats(self):
        ...
        super(ManualControlFreeChoice_HLreward, self).update_report_stats()
        self.reportstats['Targs [0,1]'] = self.targs[0,1] #HS
        self.reportstats['ts'] = 999#self.ts #HS
    ...
    def _start_wait(self):
        super(ManualControlFreeChoice_HLreward, self)._start_wait()
        ...
        #get target positions and reward assignments for this trial #HS assign color here, too?
    ...
    def _start_hold_targetL(self):
        self.reward_time = self.rewardL*self.reward_time_high + (1 - self.rewardL)*self.reward_time_low
        self.show_object(self.targetL, True)
        self.timedout = False
        self.requeue()

    def _start_hold_targetH(self):
        self.reward_time = self.rewardH*self.reward_time_high + (1 - self.rewardH)*self.reward_time_low
        self.show_object(self.targetH, True)
        self.timedout = False
        self.requeue()
    ...
    @staticmethod
    def colored_targets_with_probabilistic_reward(length=1000, boundaries=(-18,18,-10,10,-15,15),reward_high_prob=100,reward_low_prob=0):
    ...
    @staticmethod
    def colored_targets_with_different_reward_times(length=1000, boundaries=(-18,18,-10,10,-15,15)):
        ...
        The first dimension (size=ntrials) is the total number of trials (default=1000).
        The second dimension (size=2) is for each target.
            The first is the target associated with DENATONIUM - the target with the static reward time.
            The second is the target associated with JUICE ONLY - the set of 5 targets for which the reward times are different from each other but the same over time.
        The third dimension (size=3) holds three variables indicating 
            position offset (yes/no)
            juice only trial type (1,2,3,4,or 5)
            location (binary returned where the output indicates either left or right - HS: not sure if 0=L or R)
        """

        position_offsetH = np.random.randint(2,size=(1,length))
        position_offsetL = np.random.randint(2,size=(1,length))
        location_int = np.random.randint(2,size=(1,length))#*0 #HS "*0" ensures that the targets never change sides

        # coin flips for reward schedules, want this to be elementwise comparison
        #assign_rewardH = np.random.randint(0,100,size=(1,length))
        #assign_rewardL = np.random.randint(0,100,size=(1,length))

        juice_only_options = [1, 2, 3, 4, 5]#[0, 0.4, 0.6, 0.8, 1.0]
        alt_times_shuffled = []

        for i in range(int(length/5)):
            alt_times_shuffled.append(random.sample(juice_only_options,5))

        alt_times = np.array(alt_times_shuffled).reshape((1,length))
        print(alt_times)
        ...
        pairs[:,1,0] = position_offsetL
        pairs[:,1,1] = np.ones((1,length))*0.80 #HS - DENATONIUM REWARD TIME IS CONSTANT
        pairs[:,1,2] = 1 - location_int

        return pairs

    @staticmethod
    def fivetargetstask_training_seq(length=1002, boundaries=(-18,18,-10,10,-15,15)):
        ...
        The third dimension (size=3) holds three variables indicating 
            position offset (yes/no)
            juice only trial type (1,2, or 3)
            location (binary returned where the output indicates either left or right - HS: not sure if 0=L or R)
        """
        ...
        juice_only_options = [1,2,3]
        alt_times_shuffled = []

        for i in range(int(length/3)):
            alt_times_shuffled.append(random.sample(juice_only_options,3))
...
class FiveTargetsTask(ManualControlFreeChoice_HLreward):

    '''
    This is for the denatonium task in which there are five alternative juice only options with varying reward times.
    This task is not probabilistic in terms of obtaining a reward.  It is random which target appears.  Target color dictates the reward length.


    '''
    #Settable Traits
    reward_time_denatonium = traits.Float(0.8, desc="Length of reward time for denatonium (seconds)")
    reward1 = traits.Float(0.0, desc="Lowest reward time")
    reward2 = traits.Float(0.2, desc="Second lowest reward time")
    reward3 = traits.Float(0.5, desc="Middle reward time")
    reward4 = traits.Float(0.8, desc="Second highest reward time")
    reward5 = traits.Float(1.0, desc="Highest reward time")
    ...
    status = dict(
        ...
        timeout_penalty = dict(timeout_penalty_end="targ_transition"),
        hold_penalty = dict(hold_penalty_end="targ_transition"),
        reward = dict(reward_end="rest_period"),#reward = dict(reward_end="wait"),
        rest_period = dict(rest_end="wait")
   )
    ...
    def _start_center(self):
        ...
        self.targetH.color = target_colors[color_dict.get(self.targs[0,1])]
        #HS self.targetH.color = self.color_targets*self.color1 + (1 - self.color_targets)*self.color2 # high is magenta if color_targets = 1, juicyorange otherwise
        self.targetL.color = target_colors['teal']#(1 - self.color_targets)*self.color1 + self.color_targets*self.color2        
        self.show_object(self.target1, True)
        self.show_object(self.cursor, True)
    ...
    def _start_hold_targetL(self):
        self.reward_time = self.reward_time_denatonium #0.80 #HS self.rewardL*self.reward_time_high + (1 - self.rewardL)*self.reward_time_low
        self.show_object(self.targetL, True)
        self.timedout = False
        self.requeue()
    ...
    def _start_hold_targetH(self):
        ...
        self.reward_time = reward_time_dict.get(self.targs[0,1])#HS self.rewardH*self.reward_time_high + (1 - self.rewardH)*self.reward_time_low
        self.show_object(self.targetH, True)
        self.timedout = False
        self.requeue()

    def _test_rest_end(self, ts): #HS
        self.show_object(self.target1, False)
        self.show_object(self.targetL, False)
        self.show_object(self.targetH, False)
        self.show_object(self.cursor, False)
        rest_time = round(random.uniform(0.5, 1.5), 1)
        rest_ended = (ts > rest_time)
        self.task_data['random_rest_time'] = rest_time
        self.task_data['HMS_ts'] = ts
        return(rest_ended)

class FiveTargetTask_training(FiveTargetsTask,ManualControlFreeChoice_HLreward):
...
"""
class FreeChoicePilotTask(ManualControlFreeChoice_HLreward):
    '''
    ...
    def _start_pulse(self):
        ...
        comedi.comedi_dio_bitfield2(self.com, subdevice, write_mask, val, base_channel)
        self.pulse_count = self.pulse_count + 1
        print "num of pulses", self.pulse_count
        #self.stimulation_start = self.get_time() - self.start_time
    def _test_pulse_end(self,ts):
        return ts >= self.stimulation_pulse_length*1e-6
    def _end_pulse(self):
        subdevice = 0
        write_mask = 0x800000
        val = 0x000000
    ...
    def _test_another_pulse(self,ts):
        #interpulse_time = (1.0/self.stimulation_frequency) - self.stimulation_pulse_length*1e-6    # period minus the duration of a pulse
        #print 'interpulse period', interpulse_time
        return ts>=self.interpulse_time
    def _test_pulse_train_complete(self,ts):
        #number_of_pulses = np.floor(self.hold_time*self.stimulation_frequency)   # total pulses during a stimulation pulse train, assumes hold_time is in s
        return (self.pulse_count > self.number_of_pulses)     # end train if number of pulses is completed or if the animal ends holding early
    '''
    def _start_target(self):
        #self.target_index += 1
        subdevice = 0
    ...
    def _start_timeout_penalty(self):
        ...
        base_channel = 0
        comedi.comedi_dio_bitfield2(self.com, subdevice, write_mask, val, base_channel)
class InstructedChoicePilotTask(ManualControlFreeChoice_HLreward):
    ...
    def _start_reward(self):
        ...
        #self.reward_counter = self.reward_counter + float(reward_assigned)
        self.requeue()
class ManualControlFreeChoice_withReversal(ManualControlFreeChoice_HLreward):
    # create settable traits
    ...
    def _start_wait(self):
        ...
        assign_reward = np.random.randint(0,100,size=2)
        self.rewardH = np.greater(self.targs[0,1],assign_reward[0])
        self.rewardH = ((self.calc_trial_num() < self.reversal_trial_num_begin) or (self.calc_trial_num() >= self.reversal_trial_num_end))*self.rewardH + (self.reversal_trial_num_begin <= self.calc_trial_num() < self.reversal_trial_num_end)*self.rewardL
        #print 'high value target reward prob', self.targs[0,1]
        self.rewardL = np.greater(self.targs[1,1],assign_reward[1])
        self.rewardL = ((self.calc_trial_num() < self.reversal_trial_num_begin) or (self.calc_trial_num() >= self.reversal_trial_num_end))*self.rewardL + (self.reversal_trial_num_begin <= self.calc_trial_num() < self.reversal_trial_num_end)*self.rewardH
    ...
    def _test_no_reward(self,ts):
        ...
        return bool(not reward_assigned)
    '''
class ManualControlFreeChoice_PseudoRandom(ManualControlFreeChoice_HLreward):
    # create settable traits
    background = (0,0,0,1)
    shoulder_anchor = np.array([2., 0., -15.]) # Coordinates of shoulder anchor point on screen
    ...
    sequence_generators = ['colored_targets_with_block10_probabilistic_reward','block_probabilistic_reward']
    def __init__(self, *args, **kwargs):
    ...
    def colored_targets_with_block10_probabilistic_reward(length=1000, boundaries=(-18,18,-10,10,-15,15),reward_high_prob=80,reward_low_prob=40):
        ...
        for i in range(1,101):
            low_rewarded_trials = random.sample(range(10*(i-1),10*(i-1) + 10),num_low)
            high_rewarded_trials = random.sample(range(10*(i-1),10*(i-1) + 10),num_high)
